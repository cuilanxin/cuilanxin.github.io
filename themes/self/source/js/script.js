...
$$(function(){
  ...
  clxlizhi && (clxlizhi.innerHTML = clx$$$$[(Math.round(Math.random() * 61) + 1)])
})
$$(function (){
  var switchModeClx = document.getElementById('switch-mode-clx')
  var clxabiaoqian = document.getElementsByClassName('clxabiaoqian')
  var fillpath = document.getElementById('switch-mode-clx1')
  ...
  switchModeClx.addEventListener('click',()=>{
    if(localStorage.getItem("mdui-theme-layout-dark")) {
      switchModeClx && (switchModeClx.innerHTML = switchModeClxYue)
      switchModeClx && (switchModeClx.title = '切换模式-黑暗模式')
      switchModeClx && (switchModeClx.style.paddingTop = 0)
      Array.from(clxabiaoqian).forEach((item,index)=>{
        clxabiaoqian[index].style.color = '#000'
      })
      fillpath && (fillpath.style.fill = '#000')
    }else {
      switchModeClx &&  (switchModeClx.innerHTML = switchModeClxRi)
      switchModeClx && (switchModeClx.title = '切换模式-光亮模式')
      Array.from(clxabiaoqian).forEach((item,index)=>{
        clxabiaoqian[index].style.color = '#bbb'
      })
      fillpath && (fillpath.style.fill = '#fff')
    }
    clxabc()
  })
  function clxabc(){
